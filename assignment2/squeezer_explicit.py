...
class Seven_bar_mechanism_explicit(ap.Implicit_Problem):
    """
    ...
    def f(self,t, y):
        """
        Right hand side function giving derivatives to the vector y = [p v]
        """
        # Inertia data
        ...
        #   index-1 formulation of derivatives
        
        yp = zeros(14)
        yp[0:6] = y[7:13]
        
        mm1 = np.linalg.inv(m)
        yp[7:13] = mm1@(ff-(gp@m@np.transpose(gp))@(gp@mm1@ff - gqq))
        return yp
