...
class HHT_a(Explicit_ODE):
    ...
    def integrate(self, t, y, tf, opts):
        ...
        def init_HHT(t,y):
            u = y[0]
            up = y[1]
            rhs = self.f(t,u) - self.problem.C@up - self.problem.K@u     
        ...
        def HHT_step(t,y,h):
            ...
            rhs3 = self.alpha*self.problem.K@u
            rhs = f + rhs1 + rhs2 + rhs3
            
            lhs = ( self.problem.M/(self.beta*h**2) + (self.gamma*self.problem.C)/(self.beta*h) + (1 + self.alpha)*self.problem.K )
