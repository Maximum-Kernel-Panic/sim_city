...
class Explicit_Problem_2_to_1wrap():
    
    def __init__(self, rhs, t0, y0, y0d):
        self.rhs2nd = rhs
        self.y0 = y0
        self.y0d = y0d
        self.t0 = t0
        self.state0 = np.concatenate([y0,y0d])
        self.dim = len(y0)
        #print(self.dim)
    ...
    def makeModel(self):
        ...
        model = Explicit_Problem(self.buffRHS,self.state0,self.t0)
        return model
...
testprob = Explicit_Problem_2t1wrap(rhs1,0,x0,x0d)
